...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const handleLogin = async (email: string, password: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const res = await fetch(`https://smart-trace-device-backend.onrender.com/api/auth/login/`, {
        method: "POST",
  ...
  const handleSignup = async (
    ...
    setError(null);
    try {
      const res = await fetch(`https://smart-trace-device-backend.onrender.com/api/auth/register/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const verifyOtp = async (email: string, code: string) => {
    ...
    setError(null);
    try {
      const res = await axios.post("https://smart-trace-device-backend.onrender.com/api/auth/verify-email/", {
        email,
        code,
      });
      setLoading(false);
  ...
  return (
    <AuthContext.Provider
      value={{ user, loading, error, handleLogin, handleSignup, verifyOtp, resendOtp, logout }}
    >
      {children}
    </AuthContext.Provider>
  );
